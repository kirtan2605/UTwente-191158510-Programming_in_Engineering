...
void LUDecomposition(float * A,float * uppertriangle,float * lowertriangle, int n)
{
        int ctr1,ctr2;
        int row,column;
        int i,j,k;
        ...
        {       for(column=0;column<n;column++)
                {
                        if(row>column)          //initialise all elements of upper triangle in upper triangular matrix as 0
                                *(uppertriangle+row*n+column)=0.0;
                        if(row<column)          //initialise all elements of lower triangle in lower triangular matix as 0
                                *(lowertriangle+row*n+column)=0.0;
                        if(row==column)         //initialise all diagonal elements of lower triangular matrix as 1
                                *(lowertriangle+row*n+column)=1.0;
                }
        }
        ...
        {       
                for(ctr1 = m; ctr1<n;ctr1++)
                {
                        ...
                        *(lowertriangle+ctr1*n+ctr2) = mul[ctr1];
                        row = ctr1;
        ...
        {
        for(column=0;column<n;column++)
                        *(uppertriangle+row*n+column) = *(A+row*n+column);
        }
...
{ 
        ...
        LUDecomposition((float*)A,(float*)uppertriangle,(float*)lowertriangle,n);
